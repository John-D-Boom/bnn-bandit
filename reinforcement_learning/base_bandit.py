...
class Bandit():
    ...
    def update(self, mushroom):
        ...
        context_pool = torch.Tensor(np.array([self.buffer_x[i] for i in idx_pool])).to(DEVICE)
        value_pool = torch.Tensor(np.array([self.buffer_y[i] for i in idx_pool])).to(DEVICE)
